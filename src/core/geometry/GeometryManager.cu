 ...
 */
__global__ void
transformDynamicLinesKernel(const uint32_t num_dynobj_lines, const uint32_t num_envs,
  ...
  if (line_id >= num_dynobj_lines || batch_id >= num_envs)
    return;
  ...
  float2 new_vertex = {vertex.x * cost - vertex.y * sint + pose.x, vertex.x * sint + vertex.y * cost + pose.y};
...
public:
  Impl() {
    ...
    GridMapDescription grid_map_desc =
        GridMapGenerator({GRIDMAP_WIDTH, GRIDMAP_HEIGHT, {0, 0}, GRIDMAP_RESOLU}).getGridMapDescription();
    static_esdf_ = group_mgr->registerConfigTensor<float>("static_esdf",
                                                          {grid_map_desc.grid_size.y, grid_map_desc.grid_size.x, 4});
  }
...
protected:
  ...
  void assembleStaticWorld() {
    ...
    for (int64_t group_id = 0; group_id < static_scene_descs_->getNumEnvGroup(); group_id++) {
      ...
      auto poly_handle_fn = [&num_static_lines_in_group, &static_line_data]<typename T>(const T *shape,
        ...
        for (auto line_iter = shape->begin(transform); line_iter != shape->end(transform); ++line_iter) {
          ...
          static_line_data[num_static_lines_in_group++] =
              make_float4((*line_iter).start.x, (*line_iter).start.y, (*line_iter).end.x, (*line_iter).end.y);
        }
      };
  ...
  void assembleDynamicWorld() {
    ...
    auto poly_handle_fn = [&h_dyn_shape_line_ids, &h_dyn_shape_lines, &line_idx]<typename T>(const T *shape,
                                                                                             int shape_id) {
