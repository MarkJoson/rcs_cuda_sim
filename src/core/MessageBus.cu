...
const MessageShape& MessageBus::getMessageShape(const MessageNameRef& message_name) {
    return impl_->getMessageShape(message_name);
}

void MessageBus::registerInput(ExecuteNode* component, const ExecuteNode::NodeInputInfo &info) {
    impl_->registerInput(component, info);
}
...
MessageQueue* MessageBus::getMessageQueue(const NodeNameRef &node_name, const MessageNameRef &message_name) {
    return impl_->getMessageQueue(node_name, message_name);
}

void MessageBus::addTrigger(NodeTagRef trigger_tag) {
    impl_->addTrigger(trigger_tag);
}

void MessageBus::buildGraph() {
    impl_->buildGraph();
}
...
void MessageBus::trigger(NodeTagRef trigger_tag) {
    impl_->trigger(trigger_tag);
}

void MessageBus::resetExecuteOrder() {
    impl_->resetExecuteOrder();
}

int MessageBus::getCurrentExecuteOrder() const {
    return impl_->getCurrentExecuteOrder();
}

int MessageBus::getNodeOrder(NodeId node_id) const {
    return impl_->getNodeOrder(node_id);
}
