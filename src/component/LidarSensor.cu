...
void LidarSensor::onNodeReset(const core::TensorHandle &reset_flags,
...
void LidarSensor::onNodeInit() {
    ...
    std::optional<Component::NodeOutputInfo> pose_info =
        core::getContext()->getOutputInfo("robot_entry", "pose");
    ...
    addInput({"pose", input_shape, 0, core::ReduceMethod::STACK});

    addOutput({"lidar", output_shape_});
}

void LidarSensor::onNodeExecute(const core::NodeExecInputType &input,
    ...
    const float4 *dyn_lines =
        core::getGeometryManager()->getDynamicLines().typed_data<float4>();
    const float4 *static_lines =
        core::getGeometryManager()->getStaticLines().typed_data<float4>();
    const float4 *pose = input.at("pose").begin()->typed_data<float4>();
    const core::ConstantMemoryVector<uint32_t> &num_static_lines =
        core::getGeometryManager()->getNumStaticLines()->getDeviceData();

    uint32_t *lidar = output.at("lidar").typed_data<uint32_t>();

    rasterKernel<<<grid_dim, block_dim>>>(num_static_lines, static_lines,
