...
template <> struct std::hash<cuda_simulator::map_gen::impl::Point<float>> {
public:
    size_t operator()(const cuda_simulator::map_gen::impl::Point<float> &pt) const {
        return ((uint64_t)pt.x << 32 | (uint64_t)pt.y);
    }
};

template <typename T> class std::hash<cuda_simulator::map_gen::impl::Edge<T>> {
public:
    size_t operator()(const cuda_simulator::map_gen::impl::Edge<T> &e) const {
        return std::hash<cuda_simulator::map_gen::impl::Point<T>>()(e.start) ^
                     std::hash<cuda_simulator::map_gen::impl::Point<T>>()(e.start) << 1;
    }
};
