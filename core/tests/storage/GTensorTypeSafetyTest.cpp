...
TEST_F(GTensorTypeSafetyTest, TypeChecking) {
    ...
    auto* typed_tensor = dynamic_cast<RSG_SIM::GTensor<float>*>(base_tensor);
    EXPECT_NE(typed_tensor, nullptr);
    
    auto* wrong_type = dynamic_cast<RSG_SIM::GTensor<int>*>(base_tensor);
...
TEST_F(GTensorTypeSafetyTest, TypeSafeOperations) {
    ...
    auto* typed_tensor = dynamic_cast<RSG_SIM::GTensor<float>*>(tensor);
